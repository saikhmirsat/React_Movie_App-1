...
export default function Pagination({ current, onChange, total }) {
    ...
    const page = new Array(total).fill(0).map((ele, i) => <button className={current === i + 1 ? "Active" : ""} onClick={() => onChange(i + 1)} >{i + 1}</button>)

    return (
        <div className='page-btn'  >{prev} {page} {next}</div>
    )
}